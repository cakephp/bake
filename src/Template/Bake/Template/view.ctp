...
<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        ...
        <li><?= $this->Html->link(__('Edit <%= $singularHumanName %>'), ['action' => 'edit', <%= $primaryKeys %>]) ?></li>
        <li><?= $this->Form->postLink(__('Delete <%= $singularHumanName %>'), ['action' => 'delete', <%= $primaryKeys %>], ['confirm' => __('Are you sure you want to delete # {0}?', join([<%= $primaryKeys %>], ' / '))]) ?></li>
...
<% endforeach; %>
            ...
            <%- endforeach; %>
                <td class="actions">
                    <?= $this->Html->link(__('View'), ['controller' => '<%= $details['controller'] %>', 'action' => 'view', <%= $relationsPrimaryKeys %>]) %>
                    <?= $this->Html->link(__('Edit'), ['controller' => '<%= $details['controller'] %>', 'action' => 'edit', <%= $relationsPrimaryKeys %>]) %>
                    <?= $this->Form->postLink(__('Delete'), ['controller' => '<%= $details['controller'] %>', 'action' => 'delete', <%= $relationsPrimaryKeys %>], ['confirm' => __('Are you sure you want to delete # {0}?', join([<%= $relationsPrimaryKeys %>], ' / '))]) %>
                </td>
            </tr>
            <?php endforeach; ?>
        </table>
